...
const getParams = () => {
  const param1 = router.currentRoute.value.query.sortType;
  if(param1 === '1'){
    value1.value = '综合排序';
  }else if(param1 === '2'){
    value1.value = '最多播放';
  ...
  const param2 = router.currentRoute.value.query.dateRange;
  if(param2 === '1'){
    value2.value = '全部日期';
  }else if(param2 === '2'){
    value2.value = '最近一天';
  ...
  const param3 = router.currentRoute.value.query.timeSize;
  if(param3 === '1'){
    value3.value = '全部时长';
  }else if(param3 === '2'){
    value3.value = '10分钟以下';
  }else if(param3 === '3'){
    value3.value = '10-30分钟';
  }else if(param3 === '4'){
    value3.value = '30-60分钟';
  }else if(param3 === '5'){
    value3.value = '60分钟以上';
  }

  const param4 = router.currentRoute.value.query.userSort;
  if(param4 === '1'){
    value4.value = '默认排序';
  }else if(param4 === '2'){
    value4.value = '粉丝数由高到低';
  }else if(param4 === '3'){
    value4.value = '粉丝数由低到高';
  }
}

const value1 = ref('综合排序')
const options1 = ['综合排序', '最多播放', '最新发布', '最多弹幕', '最多收藏']
const value2 = ref('全部日期')
const options2 = ['全部日期', '最近一天', '最近一周', '最近半年']
const value3 = ref('全部时长')
const options3 = ['全部时长','10分钟以下', '10-30分钟', '30-60分钟', '60分钟以上']
const value4 = ref('默认排序')
const options4 = ['默认排序', '粉丝数由高到低', '粉丝数由低到高']
...
// 监听所有筛选值变化
watch([value1, value2, value3,value4], ([newVal1, newVal2, newVal3, newVal4]) => {

  if(newVal1 === '综合排序'){
    param1 = 1;
  }else if(newVal1 === '最多播放'){
    param1 = 2;
  }else if(newVal1 === '最新发布'){
    param1 = 3;
  }else if(newVal1 === '最多弹幕'){
    param1 = 4;
  }else if(newVal1 === '最多收藏'){
    param1 = 5;
  }

  if(newVal2 === '全部日期'){
    param2 = 1; 
  }else if(newVal2 === '最近一天'){
    param2 = 2;
  }else if(newVal2 === '最近一周'){
    param2 = 3; 
  }else if(newVal2 === '最近半年'){
    param2 = 4;
  }

  if(newVal3 === '全部时长'){
    param3 = 1;
  }else if(newVal3 === '10分钟以下'){
    param3 = 2; 
  }else if(newVal3 === '10-30分钟'){
    param3 = 3;
  }else if(newVal3 === '30-60分钟'){
    param3 = 4; 
  }else if(newVal3 === '60分钟以上'){
    param3 = 5;
  }

  if(newVal4 === '默认排序'){
    param4 = 1;
  }else if(newVal4 === '粉丝数由高到低'){
    param4 = 2;
  }else if(newVal4 === '粉丝数由低到高'){
    param4 = 3;
  }

  const query = { 
    ...router.currentRoute.value.query, // 保留已有参数
    sortType: param1 || undefined, // 空值时移除参数
    dateRange: param2 || undefined,
    timeSize: param3 || undefined,
    userSort: param4 || undefined
  }
...
const activeIndex = ref(currentPath)
  ...
  return views.toString();
    ...
    const getUserInfo = async () => {
      ...
      if (loginUser) {
        ...
        if(result.code === 1){
          userInfo.value = result.data;userInfo.value.followingCount = formatViews(userInfo.value.followingCount);
...
const getSerachInfo = () => {
 // 拆分路由路径，假设加密文本在路径第三段
  ...
  searchPath = pathSegments[2]
  console.log('搜索词：'+searchPath)
  ...
  try {
    // 执行解密并返回结果
    return decryptSearchText(encryptedText);
  } catch (error) {
...
const toDefault = () =>{
  ...
  // 执行路由跳转（保留当前path）
  router.push({
    path: '/search/' + searchPath + '/default',
    query: Object.fromEntries(
      Object.entries(query).filter(([_, v]) => v !== undefined)
    )
  }).then(() => {
...
const toSearch = () =>{
  ...
  const pathSegments = router.currentRoute.value.path.split('/');
  const searchType = pathSegments[3]
  const searchTextByP = encryptSearchText(searchText.value)
      router.push({path: '/search/' + searchTextByP + '/' + searchType}).then(() => {
        window.location.reload();
      });
}

const addSearchContent = async () => {
  await addSearch(searchText.value);
}

    onMounted(() => {
      searchText.value = getSerachInfo();
      getUserInfo();
      getCurrentPath();
      getParams();
...
<button class="button-upload"><svg t="1743585506476" class="upload-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="59528" width="15" height="15"><path d="M958.203854 1023.866918H65.590221A65.637783 65.637783 0 0 1 0.072874 958.229135V431.561198c0-36.191108 29.386457-65.637783 65.517347-65.637783H206.621128L180.727323 421.745639H60.291024v541.963347h903.272245V421.745639h-120.436299l-27.098168-55.822224h142.175052c36.13089 0 65.517347 29.446675 65.517346 65.577565v526.728155a65.637783 65.637783 0 0 1-65.517346 65.637783zM659.88314 230.673451L542.036221 120.654891v602.181497H481.818072V120.654891L362.586135 230.673451a36.733071 36.733071 0 0 1-59.134223-11.501667 34.806091 34.806091 0 0 1 7.888578-38.539616L485.611815 10.455677a36.733071 36.733071 0 0 1 51.245645 0l174.271325 170.176491a34.806091 34.806091 0 0 1 0 50.041283 36.913726 36.913726 0 0 1-51.245645 0z" p-id="59529" fill="#ffffff"></path></svg>
  ...
  <div class="core-search-container">
    ...
    <div class="core-search">
      <svg t="1744619047972" class="core-search-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2523" width="200" height="200"><path d="M893.64 797.29L725.18 628.84c20.55-25.29 37.36-53.22 50.14-83.42 18.11-42.81 27.29-88.27 27.29-135.1 0-46.84-9.18-92.29-27.29-135.1-17.48-41.33-42.5-78.44-74.36-110.31-31.86-31.86-68.97-56.88-110.31-74.36-42.81-18.11-88.27-27.29-135.1-27.29s-92.29 9.18-135.1 27.29c-41.33 17.48-78.44 42.5-110.31 74.36s-56.88 68.97-74.36 110.31c-18.11 42.81-27.29 88.27-27.29 135.1 0 46.84 9.18 92.29 27.29 135.1 17.48 41.33 42.5 78.44 74.36 110.31s68.97 56.88 110.31 74.36c42.81 18.11 88.27 27.29 135.1 27.29s92.29-9.18 135.1-27.29a345.135 345.135 0 0 0 83.64-50.31l168.44 168.44c7.03 7.03 16.24 10.54 25.46 10.54s18.43-3.51 25.46-10.54c14.05-14.07 14.05-36.87-0.01-50.93zM455.55 685.37c-151.67 0-275.06-123.39-275.06-275.06s123.39-275.06 275.06-275.06c151.67 0 275.06 123.39 275.06 275.06s-123.4 275.06-275.06 275.06z"  p-id="2524"></path></svg>
        ...
        >
        <button type="submit" @click="toSearch">搜索</button>
      </div>
  </div>
  ...
  <!-- 菜单 -->
   <div class="menu-container">
    ...
    @select="handleSelect"
  >
    <el-menu-item :index="`/search/${searchPath}/default`" @click="toDefault">&nbsp;&nbsp;&nbsp;综合&nbsp;&nbsp;&nbsp;</el-menu-item>
    <el-menu-item :index="`/search/${searchPath}/video`" @click="toVideo">&nbsp;&nbsp;&nbsp;视频&nbsp;&nbsp;&nbsp;</el-menu-item>
    <el-menu-item index="3" disabled>&nbsp;&nbsp;&nbsp;番剧&nbsp;&nbsp;&nbsp;</el-menu-item>
    <el-menu-item index="4" disabled>&nbsp;&nbsp;&nbsp;影视&nbsp;&nbsp;&nbsp;</el-menu-item>
    <el-menu-item index="5" disabled>&nbsp;&nbsp;&nbsp;直播&nbsp;&nbsp;&nbsp;</el-menu-item>
    <el-menu-item index="6" disabled>&nbsp;&nbsp;&nbsp;专栏&nbsp;&nbsp;&nbsp;</el-menu-item>
    <el-menu-item :index="`/search/${searchPath}/user`" @click="toUser">&nbsp;&nbsp;&nbsp;用户&nbsp;&nbsp;&nbsp;</el-menu-item>
  </el-menu>
  <el-divider class="menu-container-divider"/>
  </div>

  <!-- 筛选栏 -->
   <div class="filter-container">
    <div  v-if="currentPath != `/search/${searchPath}/user`">
    <el-segmented class="filter-container1" v-model="value1" :options="options1" size="large" />
    <el-segmented v-if="isOpen" class="filter-container2" v-model="value2" :options="options2" size="large" />
    <el-segmented v-if="isOpen" class="filter-container3" v-model="value3" :options="options3" size="large" />
  </div>
  <div v-if="currentPath === `/search/${searchPath}/user`">
    <el-segmented class="filter-container1" v-model="value4" :options="options4" size="large" />
  </div>
  </div>

  <div class="switch-container" v-if="currentPath != '/search/user'">
    <button 
    class="filter-button"
    @click="toggle"
    :aria-expanded="isOpen"
  >
    <span class="switch-container-text">更多筛选</span>
    <span 
      class="arrow"
  ...
  <div class="search-bottom">
    <img class="search-bottom-img" src="YilenaPic/25/04/14-3759544ff5834baf913478ca2d6b3d6f.jpg">
  </div>
</template>
...
.core-search button[type="submit"] {
...
.core-search button[type="submit"]:hover {
...
/* 覆盖组件内部容器间距 */
.filter-container .el-segmented {
  gap: 20px !important; /* 控制所有 el-segmented 之间的垂直间距 */
  color: rgb(112, 116, 123) !important;
  font-size: 14px;
  --el-segmented-item-selected-color: rgb(0, 174, 237);
  --el-segmented-item-selected-bg-color: rgb(223, 246, 253);
  --el-border-radius-base: 16px;
}

/* 单个分段器内部选项间距 */
.filter-container1 .el-segmented__group,
.filter-container2 .el-segmented__group,
.filter-container3 .el-segmented__group {
  gap: 25px !important; /* 选项间横向间距 */
}
