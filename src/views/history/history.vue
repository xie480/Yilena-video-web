...
import { getHistory,isRecordUser,deleteAllHistory,deleteOneHistory } from '../../api/history';
...
const historyVideoData = ref([])
const historyDateData = ref([])
const historyTimeData = ref([])
const historyIds = ref([])

const getUserHistory = async () => {
  const result = await getHistory(userInfo.value.id);
  if(result.code === 1){
    const data = result.data;;
    // 清空原有数据
    historyVideoData.value = [];
      historyDateData.value = [];
      historyTimeData.value = [];
      historyIds.value = [];
      ...
      for (const dateStr in data) {
        if (data.hasOwnProperty(dateStr)) {
          ...
          // 添加到数组
          historyDateData.value.push(dateData.dateStr);
          historyVideoData.value.push(dateData.videoVOS);
          historyTimeData.value.push(dateData.createTimes);
          historyIds.value.push(dateData.ids);
        }
      }

      for(let i = 0; i < historyVideoData.value.length; i++){
        historyVideoData.value[i] = historyVideoData.value[i].reverse(); 
      }

      for(let i = 0; i < historyTimeData.value.length; i++){
        historyTimeData.value[i] = historyTimeData.value[i].reverse();
        for(let j = 0; j < historyTimeData.value[i].length; j++){
          historyTimeData.value[i][j] = formatTime(historyTimeData.value[i][j]); 
        }
      }

      for(let i = 0; i < historyIds.value.length; i++){
        historyIds.value[i] = historyIds.value[i].reverse(); 
      }
    console.log(historyVideoData.value)
    console.log(historyDateData.value)
    console.log(historyTimeData.value)
    console.log(historyIds.value)
  }else{
...
const deleteAll = async () => {
  ElMessageBox.confirm('是否清空所有历史记录？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning', 
  }).then(async () => {
    const result = await deleteAllHistory(userInfo.value.id);
    if(result.code === 1){
...
const deleteOne = async (index,indexSon) => {
 const id = historyIds.value[index][indexSon];
 const result = await deleteOneHistory(id);
 if(result.code === 1){
...
const formatDate = (dateString) => {
    ...
    if (normalizedInput.getTime() === normalizedToday.getTime()) {
      ...
      return '昨天';
    } else if (normalizedInput.getTime() === normalizedDayBeforeYesterday.getTime()) {
      return '前天';
    } else {
      ...
      if (inputDate > oneYearAgo) {
        ...
        // 超过一年
        return dateString; // 或者格式化为 'YYYY-MM-DD'
      }
    }
  };
  ...
  const formatTime = (dateString) => {
    ...
    // 去掉秒的部分（格式：2025-05-04T10:20）
    const trimmedDateString = dateString.substring(0, 16);
    const inputDate = new Date(trimmedDateString);
    const now = new Date();
    ...
    if (isSameDay(inputDate, today)) {
        ...
        return `昨天 ${timeStr}`;
    } else if (isSameDay(inputDate, dayBeforeYesterday)) {
        return `前天 ${timeStr}`;
    } else {
        // 检查是否同年
...
const changeIsRecord = () => {
  recordEntity.userId = userInfo.value.id;
  recordEntity.username = userInfo.value.username;
  recordEntity.isRecord = isRecord.value? 1 : 0;
  isRecordUser(recordEntity);
}
        ...
        <!-- 右侧导航栏 -->
         <el-col :span="8">
          <div class="header-right">
               <!-- 头像下拉菜单 -->
    <el-dropdown>
      <el-avatar :src= "userInfo.image" :size="37" class="avatar" @click="toUserInfo"></el-avatar>
      <template #dropdown>
        <el-dropdown-menu class="custom-dropdown-menu"> 
          ...
          <el-row>
            <el-col :span="24">
<button class="cta-1" @click="toUserInfo">
...
<button class="button-upload"><svg t="1743585506476" class="upload-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="59528" width="15" height="15"><path d="M958.203854 1023.866918H65.590221A65.637783 65.637783 0 0 1 0.072874 958.229135V431.561198c0-36.191108 29.386457-65.637783 65.517347-65.637783H206.621128L180.727323 421.745639H60.291024v541.963347h903.272245V421.745639h-120.436299l-27.098168-55.822224h142.175052c36.13089 0 65.517347 29.446675 65.517346 65.577565v526.728155a65.637783 65.637783 0 0 1-65.517346 65.637783zM659.88314 230.673451L542.036221 120.654891v602.181497H481.818072V120.654891L362.586135 230.673451a36.733071 36.733071 0 0 1-59.134223-11.501667 34.806091 34.806091 0 0 1 7.888578-38.539616L485.611815 10.455677a36.733071 36.733071 0 0 1 51.245645 0l174.271325 170.176491a34.806091 34.806091 0 0 1 0 50.041283 36.913726 36.913726 0 0 1-51.245645 0z" p-id="59529" fill="#ffffff"></path></svg>
  ...
  <div class="history-title">
    <span class="history-title-text"><el-icon><Clock /></el-icon>&nbsp;&nbsp;&nbsp;历史记录</span>
    <el-row class="history-switch">
    <el-switch v-model="isRecord" />
    <span class="history-switch-text">&nbsp;&nbsp;记录浏览历史</span>
  </el-row>
  <el-button @click="deleteAll" class="delete-all-btn"><el-icon><DeleteFilled /></el-icon>&nbsp;&nbsp;清空历史</el-button>
  </div>

  <el-timeline style="max-width: 930px" class="history-timeline" v-if="historyDateData.length > 0">
    <el-timeline-item v-for="(item,index) in historyDateData" center :timestamp="formatDate(item)" placement="top">
      <el-card v-for="(item,indexSon) in historyVideoData[index]" @click="playVideo(item.id)">
        <img :src="item.coverUrl" class="history-img"/>
        <span class="history-video-time">{{ formatVideoDuration(item.time) }}</span>
        <span class="history-video-title">{{ item.title }}</span>
        <svg t="1744025410333" class="history-video-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="10955" width="200" height="200"><path d="M800 128H224C134.4 128 64 198.4 64 288v448c0 89.6 70.4 160 160 160h576c89.6 0 160-70.4 160-160V288c0-89.6-70.4-160-160-160z m96 608c0 54.4-41.6 96-96 96H224c-54.4 0-96-41.6-96-96V288c0-54.4 41.6-96 96-96h576c54.4 0 96 41.6 96 96v448z" p-id="10956"></path><path d="M419.2 544c0 51.2-3.2 108.8-83.2 108.8S252.8 595.2 252.8 544v-217.6H192v243.2c0 96 51.2 140.8 140.8 140.8 89.6 0 147.2-48 147.2-144v-240h-60.8V544zM710.4 326.4h-156.8V704h60.8v-147.2h96c102.4 0 121.6-67.2 121.6-115.2 0-44.8-19.2-115.2-121.6-115.2z m-3.2 179.2h-92.8V384h92.8c32 0 60.8 12.8 60.8 60.8 0 44.8-32 60.8-60.8 60.8z" p-id="10957"></path></svg>
    <span class="history-video-author">
      {{ item.username }}
    </span>
    <div class="history-createdTime-div">
    <span class="history-createdTime" v-for="(item,indexTime) in historyTimeData[index][indexSon]">{{ item }}</span>
  </div>
  <el-button text @click="deleteOne(index,indexSon)" class="history-del-one"><el-icon><CloseBold /></el-icon></el-button>
      </el-card>
    </el-timeline-item>
  </el-timeline>
...
.history-timeline .el-card:hover {
...
.history-timeline .el-card::before {
  content: '';
  position: absolute;
  top: 0;
  left: -100%;
...
.history-timeline .el-card:hover::before {
  left: 0;
}

/* 在全局或组件样式表中添加 */
.history-timeline .el-timeline-item:hover {
...
/* 如果需要更明显的效果 */
.history-timeline .el-timeline-item:hover .el-timeline-item__node {
  background-color: #409eff; /* 时间轴节点颜色变化 */
  transform: scale(1.2); /* 节点放大 */
}

.history-timeline .el-timeline-item:hover .el-timeline-item__timestamp {
  color: #409eff; /* 时间戳文字颜色变化 */
  font-weight: bold;
}
