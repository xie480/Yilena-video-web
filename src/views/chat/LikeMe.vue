...
import { getLikeMeMsg,getAnswerMeMsg,getUserFollowingUsers,getChatHistory,sendMessage,setMessageRead,deleteMessage } from '../../api/chat';
...
const getMsg = async () => {
  const result = await getLikeMeMsg(currentUserId.value);
  if(result.code === 1){
...
const formatMessage = (content) => {
  ...
  for (let i = 1; i < lines.length; i++) {
    if (lines[i].trim()) {  // 忽略空行
      result += `<span class="text-ellipsis" style="
  display: -webkit-box;
  -webkit-line-clamp: 3;
