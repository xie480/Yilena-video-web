...
const getUserPendingList = async () => {
  ...
  const result = await getUserList(userQueryEntity.value.username,userQueryEntity.value.beginTime,userQueryEntity.value.endTime,userQueryEntity.value.status,userQueryEntity.value.page,userQueryEntity.value.pageSize)
  if(result.code === 1){
...
const updateCurrentUserStatus = async (id, status) => {
  updateEntity.value.id = id;
  updateEntity.value.status = status;

  if (status == 0) {
    updateEntity.value.status = '1';
    const result = await updateUserStatus(updateEntity.value);
    if (result.code == 1) {
    ...
    try {
     ...
     updateEntity.value.status = '0';
      // 用户输入了原因并点击确认
      updateEntity.value.reason = reason;
      const result = await updateUserStatus(updateEntity.value);
...
<template>
 ...
 <div class="user-manage" v-else>
  ...
  <el-select
    ...
    <el-table-column align="center" fixed prop="username" label="用户名" width="120" />
    <el-table-column align="center" label="头像" width="120">
        <template #default="scope">
          <el-avatar :src="scope.row.image" />
        </template>
    </el-table-column>
    <el-table-column align="center" show-overflow-tooltip prop="description" label="简介" width="250" />
    <el-table-column align="center" prop="followingCount" label="关注数" width="120" />
    <el-table-column align="center" prop="followersCount" label="粉丝数" width="120" />
    <el-table-column align="center" prop="postCount" label="动态数" width="120" />
    <el-table-column align="center" prop="videoCount" label="视频数" width="120" />
    ...
    <el-table-column align="center" show-overflow-tooltip label="原因" width="300">
      <template #default="scope">{{ (scope.row.reason == '' || scope.row.reason == null) ? '无':scope.row.reason }}</template>
      </el-table-column>
    <el-table-column align="center" fixed="right" prop="createdTime" label="注册时间" width="180" />
    <el-table-column align="center" prop="updatedTime" label="修改时间" width="180" />
