...
const handleSelect = async(key, keyPath) => {
  ...
  for(let i = 0; i < userSubscribeFavoritesFolders.value.length; i++) {
   if(userSubscribeFavoritesFolders.value[i].id === key - 1) {
...
const getCurrentFavoritesFolderVideos = async () => {
  ...
  if(flag.value){
    searchTerm.value.id = currentFavoritesFolderId.value - 1;
    flag.value = false;
  }
  const result = await getFavoritesFolderVideos(searchTerm.value.id,searchTerm.value.title, searchTerm.value.page, searchTerm.value.pageSize,currentUserInfo.value.id);
  if (result.code === 1) {
...
const getFavoritesFolders = async () => {
  const result = await getOtherFavoritesFolder(currentUserId.value); 
  if (result.code === 1) {
...
const getIsSubscribe = async() => {
  const result = await isCurrentFavoritesFolderSubscribe(currentUserInfo.value.id,currentFavoritesFolderId.value - 1);
  if(result.code === 1){
    if(result.data != null){
...
const subscribe = async() => {
  const result = await subscribeOhterFavoritesFolder(currentFavoritesFolderId.value - 1);
  if(result.code === 1){
...
const notSubscribe = async() => {
  const result = await notSubscribeOhterFavoritesFolder(currentFavoritesFolderId.value - 1);
  if(result.code === 1){
...
<style scoped>
.userInfo-favorites-content-top-manage-btn-div{
  position: absolute;
}
