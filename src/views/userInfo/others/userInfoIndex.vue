...
import { getUserFavoritesList,getOtherUserFavoritesList } from '../../../api/video';
...
const getFavorites = async () => {
  const result = await getOtherUserFavoritesList(currentUserId.value);
  if(result.code === 1){
...
const toUfUpload = () => {
  router.push(`/userInfo/others/${currentUserId.value}/upload`).then(() => {
    window.location.reload();
  }); 
}

const toUfFavorites = () => {
  router.push(`/userInfo/others/${currentUserId.value}/favorites`).then(() => {
    window.location.reload();
  }); 
}
