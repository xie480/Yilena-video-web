...
import { updateUserInfo ,getnewToken} from '../../../api/userInfo';
...
const getCurrentPath = () => {
  ...
  path.value = pathArr[1] + '/' + pathArr[2];
...
const handlePasswordSubmit = async () => {
  try {
    ...
    updatePasswordEntity.value.id = userInfo.value.id
    updatePasswordEntity.value.password = passwordForm.newPassword;
    updatePasswordEntity.value.oldPassword = passwordForm.oldPassword;
    const result = await updateUserInfo(updatePasswordEntity.value);
    if(result.code === 1){
...
const updateCurrentUserInfo = async () => {
  const result = await updateUserInfo(updateUserInfoDTO.value);
  if(result.code === 1){
    const r = await getnewToken(userInfo.value.id);
    if(r.code === 1){
     localStorage.removeItem('loginUser');
     localStorage.setItem('loginUser', JSON.stringify(r.data));
    updateUserInfoDialogVisible.value = false;
    ElMessage.success('修改成功！');
    window.location.reload();
    }else{
      localStorage.removeItem('loginUser');
      ElMessage.error('用户登录状态失效！请重新登录！')
...
const openUpdateUserInfoDialogVisible = () => {
  updateUserInfoDialogVisible.value = true;
  updateUserInfoDTO.value.id = userInfo.value.id;
  updateUserInfoDTO.value.username = userInfo.value.username;
  updateUserInfoDTO.value.image = userInfo.value.image;
  updateUserInfoDTO.value.description = userInfo.value.description;
}
...
const handleAvatarSuccess = (response,uploadFile) => {
  updateUserInfoDTO.value.image = response.data
        ...
        <!-- 右侧导航栏 -->
         <el-col :span="8">
          <div class="header-right">
               <!-- 头像下拉菜单 -->
    <el-dropdown>
      <el-avatar :src= "userInfo.image" :size="37" class="avatar"></el-avatar>
      <template #dropdown>
...
<button class="button-upload"><svg t="1743585506476" class="upload-icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="59528" width="15" height="15"><path d="M958.203854 1023.866918H65.590221A65.637783 65.637783 0 0 1 0.072874 958.229135V431.561198c0-36.191108 29.386457-65.637783 65.517347-65.637783H206.621128L180.727323 421.745639H60.291024v541.963347h903.272245V421.745639h-120.436299l-27.098168-55.822224h142.175052c36.13089 0 65.517347 29.446675 65.517346 65.577565v526.728155a65.637783 65.637783 0 0 1-65.517346 65.637783zM659.88314 230.673451L542.036221 120.654891v602.181497H481.818072V120.654891L362.586135 230.673451a36.733071 36.733071 0 0 1-59.134223-11.501667 34.806091 34.806091 0 0 1 7.888578-38.539616L485.611815 10.455677a36.733071 36.733071 0 0 1 51.245645 0l174.271325 170.176491a34.806091 34.806091 0 0 1 0 50.041283 36.913726 36.913726 0 0 1-51.245645 0z" p-id="59529" fill="#ffffff"></path></svg>
  ...
  <div class="userInfo-div">
    ...
    <el-button class="update-userInfo" text @click="openUpdateUserInfoDialogVisible"><span class="edit-text"><el-icon><EditPen /></el-icon>编辑个人信息</span></el-button>
  </div>
  ...
  >
     ...
     <el-form-item label="昵称" prop="username">
      <el-input 
        v-model="updateUserInfoDTO.username"
    ...
    <el-form-item label="头像" prop="image" required>
      ...
      >
        <el-button class="fm-coverUrl-btn">
          <el-icon><UploadFilled /></el-icon>
        </el-button>
      </el-upload>
      <img 
        class="uf-image" 
        :src="updateUserInfoDTO.image" 
        width="100" 
        height="100"
      />
    </el-form-item>
    ...
    <el-form-item label="简介" prop="description">
      <el-input
        v-model="updateUserInfoDTO.description"
    ...
    <el-button @click="updatePasswordDialogVisible = true" class="update-pw-btn" text><span class="update-pw-btn-text">修改密码</span></el-button>
    <div class="uf-dialog-footer">
    <el-button @click="updateUserInfoDialogVisible = false">取消</el-button>
    <el-button type="primary" @click="updateCurrentUserInfo">修改</el-button>
  </div>
  </el-dialog>
