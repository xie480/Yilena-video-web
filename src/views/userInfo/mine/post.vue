...
import { updatePost,deletePost } from '../../../api/userInfo';
...
const updateCurrentPost = async() => {
  currentPost.value.visibility = postVisibility.value;
  const result = await updatePost(currentPost.value);
  if(result.code === 1){
    ElMessage.success('修改成功！');
    updatePostDialogVisible.value = false;
...
const postVisibility = ref('1');

const openUpdatePostDialog = (id) => {
  updatePostDialogVisible.value = true;
  currentPost.value.id = id;
  for(let i = 0;i < userpostList.value.length;i++){
    if(userpostList.value[i].id === id){
      console.log('当前动态：',userpostList.value[i])
      currentPost.value.title = userpostList.value[i].title;
      currentPost.value.content = userpostList.value[i].content;
      currentPost.value.tags = userpostList.value[i].tags;	
      console.log('当前动态可见性：',userpostList.value[i].visibility)
      if(userpostList.value[i].visibility === 1){
        postVisibility.value = '1';
      }else{
        postVisibility.value = '0';
      }
      currentPost.value.imageUrl = userpostList.value[i].imageUrl;
      currentPost.value.userId = currentUserId.value;
      console.log('当前实体：',currentPost.value)
    }	
  }
}

const updatePostDialogVisible = ref(false);
...
const handleSuccess = (response,uploadFile) => {
  currentPost.value.imageUrl.push(response.data);
...
<div class="spinner">
  ...
  <div class="middle">loading</div>
      ...
      >
      <template #default>
    <el-row><el-button type="primary" size="small" @click="openUpdatePostDialog(item.id)">编辑</el-button>
    <el-button type="danger" size="small" @click="deleteCurrentPost(item.id)">删除</el-button></el-row>
  </template>
        <template #reference>
...
<el-dialog
  ...
  <el-dialog
    v-model="updatePostDialogVisible"
    title="修改动态"
    width="500"
  >
  <el-form :model="currentPost" label-width="auto">
  ...
  />
    ...
    <el-form-item label="图片">
      ...
      <template  v-for="(item, index) in currentPost.imageUrl" :key="index" >
  <el-row class="update-pic-show">
    <img class="post-img-show-item" :src="item" alt="图片" />
  <button class="post-img-show-item-delete-btn" @click="deleteImage(index)"><el-icon class="post-icon-cha" style="color: black;"><Close /></el-icon></button></el-row>
</template>
    </el-form-item>
    <el-form-item label="标签">
      <el-input-tag draggable class="post-center-top-input-tag" v-model="currentPost.tags" :max="3" placeholder="最多输入3个标签，按下回车键确认" />
    </el-form-item>
    <el-form-item label="可见权限">
      <el-radio-group v-model="postVisibility" :key="postVisibility">
    ...
    <template #footer>
      <div class="dialog-footer">
        <el-button @click="updatePostDialogVisible = false">取消</el-button>
        <el-button type="primary" @click="updateCurrentPost">
          确认
        </el-button>
      </div>
    </template>
...
/* 全局样式 */
.el-popover {
  min-width: 100px !important; /* 覆盖默认最小宽度 */
